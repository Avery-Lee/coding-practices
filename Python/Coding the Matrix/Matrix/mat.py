...
def equal(A, B):
    ...
    for row, col in {A.f.keys() | B.f.keys()}:
...
def scalar_mul(M, x):
    ...
    True
    """
    return Mat((M.D[0], M.D[1]), {k : x * M[k] for k in M.f.keys()})
...
def vector_matrix_mul(v, M):
    ...
    v_m_mul_result = Vec(M.D[1], {})

    for i,j in M.f.keys():
        v_m_mul_result[j] += M[i,j] * v[i]
    return v_m_mul_result
...
def matrix_vector_mul(M, v):
    ...
    m_v_mul_result = Vec(M.D[0], {})
    
    for i,j in M.f.keys():
        m_v_mul_result[i] += M[i,j] * v[j]
    
    return m_v_mul_result
...
def matrix_matrix_mul(A, B):
    ...
    for B_col in B.D[1]:
        AB_cols[B_col] = A * Vec(B.D[0], {B_row: B[B_row, B_col] for B_row in B.D[0]})
    
    return Mat((A.D[0], B.D[1]), {(row, col):AB_cols[col][row] for col in AB_cols.keys() for row in A.D[0]})
